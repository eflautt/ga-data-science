...
### Data Source
Today we will use advertising data from an example in the 
...
Understanding the difference between causation and correlation is critical both at the beginning of our workflow for the 1. identify and 2. acquire stages, when we make sure that we fully article our question and use the right data to answer it, by including any potential confounders. Additionally, this topic comes back for the final step in our data science work flow when we present the results- it is important that we present our findings in a way that doesn't overstate what our model actually measured. (e.g. saying caused when we really measured and association)
...
One such attempt that is commonly used in the medical/health sciences field is based on the work by a Bradford Hill. He developed a list of "tests" that an analysis much pass in order to indicate that there is a causal relationship. 
...
Most commonly we find an association between two variables. This means that we observe a correlation between our variables but we may not fully understand the causal direction or the other factors that may be influencing the association we are observing. 
...
#### Confounding 
Often times, the observation that we observe may be influenced by another factor.
...
It always includes at least one exposure/predictor and one outcome. 
For example: 
TV Ads --> Sales
...
Great- so let's take a look at the association between TV Ads and Sales by taking into account seasonality. 

On a DAG it would look something like (a triangle with:  TV--> Sales and seasonality --> TV Ads and seasonality --> Sales and a 2nd DAG swapping out TV with Google ads). 
...
For example- if we were working on sales data, how would we know if there was a difference between the buying patterns of men and women at Acme Inc? Hypothesis testing!
...
Typically we use a cut point of 5%. In other words, we say that something is statistically significant if there is a less than 5% chance that our finding was do to chance alone. When data scientists present results and say we found a significant result- it is almost always using these criteria. Let's dive into them further to understand p-values and confidence intervals. 
